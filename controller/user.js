...
exports.getAtt = (req, res, next) => {
  if (req.session.username) {
    userDB.getClockInOutByUser(req.session.username, (data) => {
      ...
      const state = data.length > 0 ? data[data.length - 1].out !== null : true;  //? !== ternary expression
      // render page
      res.render("att", {
        // 5am above means button will appear
...
exports.postAtt = (req, res, next) => {
  if (req.session.username) {
    //check if clock in db func or else clock out db func
    req.body.status === "in"
      ? userDB
...
exports.getLogout = (req, res, next) => {
  // log out by simply delete session data
  req.session.destroy();
...
exports.getEleave = async (req, res, next) => {
  ...
  if (req.session.username) {
    ...
    const userLeaves = await  allLeaves.filter((data) => {
    ...
    // total up approved med leaves to cal and render on page
    const totalMed = countMedLeaves.reduce((init, currentVal) => {
      return (
        init +                                                                  // init = initiate in JS
    ...
    // total up approved annual leaves to cal and render on page
    ...
    // total up approved hospitalization leaves to cal and render on page
    ...
    // total up approved martinary leaves to cal and render on page
    ...
    await delay(1000);
    // rendering with infos gather from the above
    res.render("eleave", {
      role: req.session.role,
      username: req.session.fn,
      // all leaves data for admin usage
      allLeaves: allLeaves,
...
// why doing the same function twice
      //render page with retrieved datas
      await delay(500);
      res.render("eclaim", {
        role: req.session.role,
...
exports.postClaim = async (req, res, next) => {
  ...
  if (req.session.username) {
    ...
    upload(req, res, async function (err) {
      ...
      // the datas will be push into db
      userDB.postClaim(
        req.session.rowId,
...
exports.getSetting = async (req, res, next) => {
  ...
  userDB.getDetailsByUsername(req.session.username, (userDetails) => {
    // check logged in ?
    if (req.session.username) {
      // save data in a JSON object to render on page (firstname,lastname,user id, employee id)
      const details = {
...
exports.getPay = async (req, res, next) => {
  ...
  if (req.session.username) {
    ...
    res.render("payroll", {
      ...
      additional:
        userSalary.additional === undefined ? 0 : userSalary.additional,  // why undefined
      // years
      years: years,
...
exports.payslip = async (req, res, next) => {
  if (req.session.username) {
    ...
    // get total number of OT cico from cico by user var
    const OT = await cicoByUser.filter((data) => {
      return moment(data.out).diff(moment(data.in), "hours") > 9;
    }).length;

    // get all payroll data from db
    const allPayroll = await userDB.getPayroll();
    ...
    // calc one day salary
    const onedaysalary =
      +JSON.parse(payrollByUser[0].salary).salary / moment().daysInMonth();  //where to know one day amount
